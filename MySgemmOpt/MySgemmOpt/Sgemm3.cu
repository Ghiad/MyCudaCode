...
void invokSgemm3() {
	...
	dim3 gridsize(m / BLOCK_SIZE_M/ THREAD_SIZE_Y, n / BLOCK_SIZE_N/ THREAD_SIZE_Y);
	dim3 blocksize(BLOCK_SIZE_M , BLOCK_SIZE_K);
	...
	for (int run = 0; run < nIter; run++) {
		Sgemm3<BLOCK_SIZE_M, THREAD_SIZE_Y> << <gridsize, blocksize >> > (dA, dB, dC, m, n, k);
	}
