...
#pragma unroll
	...
	FETCH_FLOAT4(frag_A[0][0]) = FETCH_FLOAT4(As[0][0][ty * THREAD_SIZE_Y / 2]);
	FETCH_FLOAT4(frag_A[0][4]) = FETCH_FLOAT4(As[0][0][ty * THREAD_SIZE_Y / 2 + 64]);

	FETCH_FLOAT4(frag_B[0][0]) = FETCH_FLOAT4(Bs[0][0][tx * THREAD_SIZE_X / 2]);
	FETCH_FLOAT4(frag_B[0][4]) = FETCH_FLOAT4(Bs[0][0][tx * THREAD_SIZE_X / 2 + 64]);
...
#pragma unroll
		for (int j = 0; j < BLOCK_SIZE_K - 1; j++) {
			//加载下一次小迭代要用的数据
			FETCH_FLOAT4(frag_A[(j + 1) % 2][0]) = FETCH_FLOAT4(As[load_flag][j + 1][ty * THREAD_SIZE_Y / 2]);
			FETCH_FLOAT4(frag_A[(j + 1) % 2][4]) = FETCH_FLOAT4(As[load_flag][j + 1][ty * THREAD_SIZE_Y / 2 + 64]);

			FETCH_FLOAT4(frag_B[(j + 1) % 2][0]) = FETCH_FLOAT4(Bs[load_flag][j + 1][tx * THREAD_SIZE_X / 2]);
			FETCH_FLOAT4(frag_B[(j + 1) % 2][4]) = FETCH_FLOAT4(Bs[load_flag][j + 1][tx * THREAD_SIZE_X / 2 + 64]);
#pragma unroll
...
#pragma unroll
		...
		FETCH_FLOAT4(frag_A[0][0]) = FETCH_FLOAT4(As[load_flag ^ 1][0][ty * THREAD_SIZE_Y / 2]);
		FETCH_FLOAT4(frag_A[0][4]) = FETCH_FLOAT4(As[load_flag ^ 1][0][ty * THREAD_SIZE_Y / 2 + 64]);

		FETCH_FLOAT4(frag_B[0][0]) = FETCH_FLOAT4(Bs[load_flag ^ 1][0][tx * THREAD_SIZE_X / 2]);
		FETCH_FLOAT4(frag_B[0][4]) = FETCH_FLOAT4(Bs[load_flag ^ 1][0][tx * THREAD_SIZE_X / 2 + 64]);
#pragma unroll
		for (int m = 0; m < THREAD_SIZE_Y; m++) {
#pragma unroll
			for (int n = 0; n < THREAD_SIZE_X; n++) {
...
void invokSgemm5() {
	...
	for (int run = 0; run < nIter; run++) {
		Sgemm5<BLOCK_SIZE_M, BLOCK_SIZE_N, BLOCK_SIZE_K, THREAD_SIZE_Y, THREAD_SIZE_X> << <gridsize, blocksize >> > (dA, dB, dC, m, n, k);
	...
	printf("My gemm5 Performance= %.2f GFlop/s, Time= %.3f msec, Size= %.0f Ops,\n",
		gigaFlops[0],
		msecPerMatrixMul[0],
		flopsPerMatrixMul);
